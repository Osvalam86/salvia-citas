 ...
 * persistente). Deja la página en `to` recargada y al final.
 */
export async function clientNavigation(b, { from, link, to, state = 'null' }) {
  await b.go(from)
  ...
  await b.click(x, y)
  for (let i = 0; i < 50 && (await b.ev('location.pathname')) !== to; i++) await sleep(100)
  await b.ev('document.fonts.ready.then(() => true)')
  await sleep(400)
  await toBottom(b)
  const arrival = await b.ev(`({ ruta: location.pathname, sinRecarga: performance.getEntriesByType('navigation')[0].name.endsWith(${JSON.stringify(from)}), estado: ${state} })`)
